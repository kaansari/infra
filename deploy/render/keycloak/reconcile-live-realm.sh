...
admin_user="${CEERAT_KEYCLOAK_ADMIN_USERNAME:-${KC_BOOTSTRAP_ADMIN_USERNAME:?set CEERAT_KEYCLOAK_ADMIN_USERNAME or KC_BOOTSTRAP_ADMIN_USERNAME}}"
admin_password="${CEERAT_KEYCLOAK_ADMIN_PASSWORD:-${KC_BOOTSTRAP_ADMIN_PASSWORD:?set CEERAT_KEYCLOAK_ADMIN_PASSWORD or KC_BOOTSTRAP_ADMIN_PASSWORD}}"
...
assign_browser_optional_scopes() {
  ...
  if [[ "$client_id" == "ceerat-admin-ui" ]]; then
    scopes="ceerat.admin.read ceerat.admin.users.read ceerat.admin.users.write ceerat.admin.rbac.read ceerat.admin.rbac.write ceerat.admin.operations.write"
  elif [[ "$client_id" == "ceerat-web-ui" ]]; then
...
reconcile_client() {
  ...
  if [[ "$client_id" == "ceerat-admin-ui" || "$client_id" == "ceerat-web-ui" || "$client_id" == "ceerat-customer-ui" ]]; then
