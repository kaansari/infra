...
.PHONY: all build-customer-ui start-customer-ui stop-customer-ui build-admin-ui start-admin-ui stop-admin-ui start-stack stop-stack status-stack ensure-dirs verify-builder verify-tools verify-coverage verify-staticcheck verify-code verify-platform k8-context k8-build k8-push k8-deploy k8-render start-k8 stop-k8 status-k8 k8-logs
